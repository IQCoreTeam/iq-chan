...
required. The handler uses `route.server.ts`, included through Next's
`pageExtensions` only in server builds. `STATIC_EXPORT=1 npm run build` omits
that handler and still exports the existing app; mirror share controls point
to the matching public server. No dynamic cards are generated by a static host.
Existing `#/...` URLs cannot provide post-specific metadata because
...
Also run `STATIC_EXPORT=1 npm run build` to verify the static/on-chain export.
Use a separate checkout for these builds when a local dev server is running,
because both build modes write to `.next`.
